...
public class EmojiUtil {
    ...
    public static void handlerEmojiText(TextView comment, String content, Context context) throws IOException {
        if(content==null||comment==null || context==null){
            return;
        }
    ...
    public static void displayTextView(EditText editText, String emojiString,Context context) {
        try {
            EmojiUtil.handlerEmojiText(editText, emojiString, context);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    public static void displayTextView(TextView textView, String emojiString,Context context) {
        try {
            EmojiUtil.handlerEmojiText(textView, emojiString, context);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }

    public static int deleteEmoji(EditText editText,int end){
        String text = editText.getText().toString().substring(0,end);
        if (text.isEmpty()) {
            return 0;
        }

        if("]".equals(text.substring(end-1))){
            ...
            editText.getText().delete(index, end);
            return end-index;
        }
